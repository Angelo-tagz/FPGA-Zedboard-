...
#define FFT_Size	   64
#define DMA_IDLE_MASK  0x00000002			// Idle mask
...
#define CONV_SCALE_FACTOR	 1.25f/4095.0f		// Conversion scale factor. 1.25 for reference voltage and 4095 for 12-bit adc
...
static int SysMonPolledPrintfExample(u16 SysMonDeviceId);			// This function implements both ADC and FFT.
static int XAdcFractionToInt(float FloatNum);						// This function is fully inherited, I did not modify anything. It presents a fraction as a full real number
static float square_num(float x);									// This function square the input number.
float my_sqrtf(float number);										// This function does square root of the input number.


/************************** Variable Definitions ****************************/

static XSysMon SysMonInst;  /* SysMon driver instance */			// Using sysmon driver instead of XAdcPs. It has more functions available for use.
XAxiDma dmaInst;			/* DMA driver instance */



u32 checkDMAIdle(u32 BaseAddress, u32 offset)						// This function is full inherited. It checks if the DMA is busy or not
{
...
/* The main is fully inherited, with slight changes in names
 * of the functions and printed sentences
 * */
int main()
{
    ...
    Status = SysMonPolledPrintfExample(SYSMON_DEVICE_ID);			// Calling the function that does XADC and FFT
...
int SysMonPolledPrintfExample(u16 SysMonDeviceId)
{
    ...
    /* Initializing the SysMon driver */
    ...
    /* Configuring to Single Channel Mode */
    XSysMon_SetSequencerMode(SysMonInstPtr, XSM_SEQ_MODE_SAFE);					// Reseting the channel
    XSysMon_SetSequencerMode(SysMonInstPtr, XSM_SEQ_MODE_SINGCHAN);				// Setting the sequencing mode

    /* Setting single channel parameters for VP/VN (no event mode, differential) */
    ...
    while (1) {
        ...
        /* Reading VP/VN Data. Raw data is shifted to match the 12-bit ADC */
        ...
        FFT_input[SampleIndex] = VpvN_Voltage;					// Put the voltage into the buffer until 64 samples (batch size)
        SampleIndex++;
      ...
      /************* This part of code is fully inherited (see project report references) *****************************************/

        	 Xil_DCacheFlushRange((UINTPTR)FFT_input, (sizeof(float complex)*FFT_Size));   		// The FFT input/output was at an address where the DMA could not retrieve it (cache), and therefore flushed
        	 ...
        	 Status = XAxiDma_SimpleTransfer(&dmaInst,(UINTPTR)FFT_output, (sizeof(float complex)) *FFT_Size, XAXIDMA_DEVICE_TO_DMA);
        	    ...
        	    //Inform DMA to send data to FFT
      ...
      /*********************************************************************/
             ...
             printf("{FFT:");
                for (int i = 0; i < SAMPLE_BATCH_SIZE; i++) {
                    float magnitude = my_sqrtf(square_num(crealf(FFT_output[i])) + square_num(cimagf(FFT_output[i])));  // finding the magnitude of FFT output(real + i * imag) sqrt((real)^2 + (imag)^2)
        ...
        usleep(5000);  /* Sleep 5ms to avoid flooding */
    }
...
/* Math.h seemed to not be functioning right,
 * therefore implemented a square and square
 * root function
 * */
